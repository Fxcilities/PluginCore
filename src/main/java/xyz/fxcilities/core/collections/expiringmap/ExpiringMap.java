...
public class ExpiringMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public static final class Builder<K, V> {
         ...
         * #variableExpiration() variable expiration}. Either an EntryLoader or ExpiringEntryLoader
         * may be set, not both.
         *
         * @param loader to set
         * @throws NullPointerException if {@code loader} is null
        ...
        public <K1 extends K, V1 extends V> Builder<K1, V1> asyncExpirationListeners(
            ...
            if (asyncExpirationListeners == null)
                asyncExpirationListeners =
                        new ArrayList<ExpirationListener<K, V>>(listeners.size());
            for (ExpirationListener<? super K1, ? super V1> listener : listeners)
                asyncExpirationListeners.add((ExpirationListener<K, V>) listener);
            return (Builder<K1, V1>) this;
        }
    ...
    private static class EntryTreeHashMap<K, V> extends HashMap<K, ExpiringEntry<K, V>>
        ...
        final class EntryIterator extends AbstractHashIterator
                implements Iterator<Map.Entry<K, V>> {
            public final Map.Entry<K, V> next() {
                return mapEntryFor(getNext());
            }
    ...
    public synchronized void addAsyncExpirationListener(ExpirationListener<K, V> listener) {
        ...
        // If asyncListener was not added on Builder, LISTENER_SERVICE was not initialized and
        // remain
        // null
                ...
                entry =
                        new ExpiringEntry<K, V>(
                                ...
                                variableExpiration
                                        ...
                                        : this.expirationPolicy,
                                variableExpiration
                                        ? new AtomicLong(expirationNanos)
    ...
    void scheduleEntry(ExpiringEntry<K, V> entry) {
        ...
        synchronized (entry) {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    Iterator<ExpiringEntry<K, V>> iterator =
                                    ...
                                    while (iterator.hasNext() && schedulePending) {
                                        ...
                                        if (nextEntry.expectedExpiration.get()
                                                <= System.nanoTime()) {
                                            iterator.remove();
            ...
            Future<?> entryFuture =
                    EXPIRER.schedule(
                            runnable,
                            entry.expectedExpiration.get() - System.nanoTime(),
                            TimeUnit.NANOSECONDS);
