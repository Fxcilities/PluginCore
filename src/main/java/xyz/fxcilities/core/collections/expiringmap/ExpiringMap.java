...
public class ExpiringMap<K, V> implements ConcurrentMap<K, V> {
  ...
  /**
   * Sets the {@link ThreadFactory} that is used to create expiration and listener callback threads
   * for all ExpiringMap instances.
   *
   * @param threadFactory
  ...
  private ExpiringMap(final Builder<K, V> builder) {
    ...
    if (LISTENER_SERVICE == null && builder.asyncExpirationListeners != null) initListenerService();
  ...
  public static final class Builder<K, V> {
    ...
    public <K1 extends K, V1 extends V> Builder<K1, V1> entryLoader(
      ...
      assertNoLoaderSet();
      entryLoader = (EntryLoader<K, V>) Checks.nonNull(loader, "loader");
      return (Builder<K1, V1>) this;
    }

    /**
     * Sets the ExpiringEntryLoader to use when loading entries and configures {@link
     * #variableExpiration() variable expiration}. Either an EntryLoader or ExpiringEntryLoader may
     * be set, not both.
     *
     * @param loader to set
     * @throws NullPointerException if {@code loader} is null
    ...
    public <K1 extends K, V1 extends V> Builder<K1, V1> expiringEntryLoader(
      ...
      assertNoLoaderSet();
      expiringEntryLoader = (ExpiringEntryLoader<K, V>) Checks.nonNull(loader, "loader");
      variableExpiration();
      return (Builder<K1, V1>) this;
    }
    ...
    /**
     * Configures the expiration listeners which will receive notifications upon each map entry's
     * expiration. Notifications are delivered synchronously and block map write operations.
     *
    ...
    /**
     * Configures the expiration listener which will receive asynchronous notifications upon each
    ...
    /**
     * Configures the expiration listeners which will receive asynchronous notifications upon each
     * map entry's expiration.
     *
     * @param listeners to set
     * @throws NullPointerException if {@code listener} is null
    ...
    private void assertNoLoaderSet() {
      Checks.state(
          entryLoader == null && expiringEntryLoader == null,
  ...
  private static class EntryLinkedHashMap<K, V> extends LinkedHashMap<K, ExpiringEntry<K, V>>
    ...
    public final class EntryIterator extends AbstractHashIterator
  ...
  private static class EntryTreeHashMap<K, V> extends HashMap<K, ExpiringEntry<K, V>>
      implements EntryMap<K, V> {
    private static final long serialVersionUID = 1L;
    ...
    final class ExpiringEntryIterator extends AbstractHashIterator
  ...
  public synchronized void addAsyncExpirationListener(ExpirationListener<K, V> listener) {
    ...
    // If asyncListener was not added on Builder, LISTENER_SERVICE was not initialized and remain
    // null
    if (LISTENER_SERVICE == null) initListenerService();
  }
  ...
  public void clear() {
    writeLock.lock();
    try {
      for (ExpiringEntry<K, V> entry : entries.values()) entry.cancel();
      entries.clear();
    } finally {
      writeLock.unlock();
    }
    ...
    try {
      ...
      if (entryLoader != null) {
        ...
        if (expiringValue == null) {
          ...
          long duration =
              expiringValue.getTimeUnit() == null
                  ? expirationNanos.get()
  ...
  /**
   * Sets the global expiration policy for the map. Individual expiration policies may override the
   * global policy.
   *
   * @param expirationPolicy
      ...
      if (entry == null) {
        ...
        if (!ExpirationPolicy.ACCESSED.equals(expirationPolicy)
            && ((oldValue == null && value == null)
                || (oldValue != null && oldValue.equals(value)))) return value;
  ...
  void scheduleEntry(ExpiringEntry<K, V> entry) {
    ...
    synchronized (entry) {
      ...
      final WeakReference<ExpiringEntry<K, V>> entryReference =
          new WeakReference<ExpiringEntry<K, V>>(entry);
      runnable =
          new Runnable() {
            @Override
            public void run() {
      ...
      Future<?> entryFuture =
          EXPIRER.schedule(
              runnable, entry.expectedExpiration.get() - System.nanoTime(), TimeUnit.NANOSECONDS);
